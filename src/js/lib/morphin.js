...
function morphin() {
  ...
  class MorphingBG {
    constructor(el) {
      this.DOM = {};
      this.DOM.el = el;
      this.wrapper = document.getElementsByClassName('js-hero-section')[0];
      ...
      this.win = {width: this.wrapper.clientWidth, height: this.wrapper.clientHeight};
      this.animate();
      this.initEvents();
    }
      ...
      const onMouseMoveFn = (ev) => {
        requestAnimationFrame(() => {
          if (!this.started) {
            ...
            const mousepos = getMousePos(ev, this.win.height);
